...
StereoFrame QuadratureMode::Process(StereoFrame input, const ParamSet& params) {
    ...
    float re = input.mono(), im = 0.0f;
    if (sub_mode_ != 0) {
        auto [r, i] = hilbert_.Process(input.mono());
        re = r; im = i;
    }
    ...
    switch (sub_mode_) {
        case 0: {
            ...
            const float ring = input.mono() * cos_c;
            const float p1   = params.p1;
            return {
                ring,
                ring * (1.0f - p1) + (input.mono() * (-sin_c)) * p1,
            };
        }
